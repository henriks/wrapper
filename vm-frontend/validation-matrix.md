...
| Wrapper TUI | Default TUI mode, `--no-tui` plain fallback, startup dialog, Codex state selection, viewport sizing from frame interior, guest key/paste routing, prompt focus isolation, resize frames, terminal cleanup. | Fast offline unit; manual live KVM/TUI smoke | `wra-3vmi` |
