...
| Wrapper contract | Absolute project/run dirs, `--setup-tool`, rejected legacy flags, public egress default, `--no-net`, `--reset`, payload env, HOME/XDG, Docker env. | Fast offline unit; live KVM/QEMU | `wra-jwaz`, `wra-9udq` |
| Wrapper TUI | Default TUI mode, unconfigured bash fallback, `--no-tui` plain fallback, explicit Codex state setup, viewport sizing from frame interior, guest key/paste routing, prompt focus isolation, resize frames, terminal cleanup. | Fast offline unit; manual live KVM/TUI smoke | `wra-3vmi` |
| Config fs and secrets | Guest-visible config files, composed bind config, hidden workspace `.sandbox`, CA cert bundle, no CA private key in guest config fs, no accidental host secret mounts. | Fast offline unit; live KVM/QEMU | `wra-s8nn`, `wra-jwaz`, `wra-9udq` |
