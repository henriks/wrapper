...
impl RuntimePaths {
    pub fn under(run_dir: impl Into<PathBuf>) -> Self {
        ...
        Self {
            ...
            data_disk: run_dir.join("docker-data.raw"),
            ...
            config_fs_sock: run_dir.join("config.sock"),
...
impl FrontendConfig {
    pub fn supervisor_plan(&self) -> SupervisorPlan {
        SupervisorPlan {
            ...
            vmnet: ManagedTask::VmnetGateway {
...
pub enum ManagedTask {
    ...
    VmnetGateway { socket_path: PathBuf },
...
mod tests {
    ...
    fn supervisor_plan_has_explicit_managed_tasks_and_logs() {
        ...
        assert!(matches!(plan.vmnet, ManagedTask::VmnetGateway { .. }));
        match plan.qemu {
