...
use agentvm_frontend::launch::{
    ...
    run_frontend_until_qemu_exit_with_policy_and_timeout_async, start_frontend_with_policy,
    LaunchError, QemuExit,
};
...
use config::{
    read_wrapper_sandbox_config, validate_share_shadow_path, wrapper_mise_config_path,
...
use launch_cli::{
    ensure_payload_listener, frontend_artifact_summary, frontend_config_from_args,
    guest_payload_env, policy_from_args, reset_project, run_launch, run_launch_async,
    runtime_mounts, validate_no_net_args, wait_for_payload_ready, LaunchCliError, PolicyArgs,
    ProjectLock,
};
#[cfg(test)]
use launch_cli::{launch_payload_args, wait_for_payload_ready_with_probe};
#[cfg(test)]
...
use wrapper::{
    ...
    WrapperCommandOverride,
};
use wrapper::{run_wrapper, WrapperError, WrapperUiMode};
...
async fn run_cli_async(argv: Vec<String>) -> CliResult<()> {
    ...
    if is_agentvm_program(&program) {
        return match args.first().map(String::as_str) {
            Some("prepare" | "vmnet-gateway" | "payload-client") => run(args),
...
#[allow(dead_code)]
fn run_cli(argv: Vec<String>) -> CliResult<()> {
    let program = argv
        .first()
        .cloned()
        .unwrap_or_else(|| "agentvm-frontend".to_string());
    let args: Vec<String> = argv.into_iter().skip(1).collect();
    debug!(program, argc = args.len(), "dispatching CLI argv");
    if is_agentvm_program(&program) {
        return match args.first().map(String::as_str) {
            Some("prepare" | "launch" | "vmnet-gateway" | "payload-client") => run(args),
            Some("self-test") => Err(CliError::UnknownCommand {
                command: "self-test".to_string(),
            }),
            _ => run_wrapper(args).map_err(Into::into),
...
fn run(args: impl IntoIterator<Item = String>) -> CliResult<()> {
    ...
    match args.first().map(String::as_str) {
        Some("vmnet-gateway") => {
            ...
            serve_vmnet_gateway(config).map_err(|error| CliError::VmnetGateway { error })?;
            Ok(())
        }
        Some("payload-client") => {
        ...
        Some("self-test") => run_self_test(&args[1..]).map_err(CliError::SelfTest),
        Some("prepare") => {
        ...
        Some("launch") => {
            info!(ui_mode = "plain", "starting launch command");
            run_launch(&args[1..], WrapperUiMode::Plain).map_err(Into::into)
        }
        Some("-h" | "--help") | None => {
            print_usage();
            Ok(())
...
mod tests {
    ...
    #[test]
    fn agentvm_argv0_does_not_expose_self_test_command() {
        assert_eq!(
            run_cli(vec!["agentvm".to_string(), "self-test".to_string()])
    ...
    #[test]
    fn wrap_subcommand_is_not_a_compatibility_entrypoint() {
        assert_eq!(
            run_cli(vec!["agentvm-frontend".to_string(), "wrap".to_string()])
                .expect_err("agentvm-frontend wrap removed")
                .to_string(),
            "unknown command: wrap"
        );
        let error = run_cli(vec!["agentvm".to_string(), "wrap".to_string()])
