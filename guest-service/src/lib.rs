//! Rust skeleton for the future AgentVM guest service.
//!
//! This crate is intentionally not installed into the appliance yet.  It starts
//! by pinning the payload frame protocol used by `docker/guest-payload-server.py`
//! and `vm-frontend/src/payload_client.rs` so a later Tokio service can be built
//! against tested compatibility semantics instead of rediscovering them during
//! the packaging migration.
...
impl FrameDecoder {
    ...
    fn peek_header(&self) -> Result<Option<(FrameKind, usize)>, FrameError> {
        ...
        let kind = FrameKind::from_byte(self.buffer[0]);
